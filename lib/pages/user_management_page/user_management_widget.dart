...
import 'user_form_dialog_adm.dart';
...
class _UserManagementWidgetState extends State<UserManagementWidget> {
  ...
  List<UserProfileModel> _getFilteredUsers(List<UserProfileModel> allUsers) {
    ...
    return allUsers.where((user) {
      ...
      // 🔹 Exclure les utilisateurs de rôle "Ame"
      final isNotAme = user.role.toLowerCase() != 'ame';

      return isNotAme && (isAdmin
          ? matchesQuery
        ...
        child: SizedBox(
          ...
          child: SingleChildScrollView(
            child: PaginatedDataTable(
              header: Text(
                "Liste des membres de l'adminstation",
                style: TextStyle(
                    fontSize: 18,
  ...
  void _openUserForm(BuildContext context, {UserProfileModel? user}) {
    showDialog(
      context: context,
      builder: (_) => UserFormDialogAdm(user: user),
    );
  }

  // _sort and _getFieldValue removed (unused)
